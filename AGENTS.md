...
Skillet 是产品层——组装 SkillRank（发现）+ Prism（图谱）+ 平台 SDK（发布），
向创作者提供端到端的内容工作流。
...
5. **Separation of concerns**: Skillet does NOT re-implement ranking (SkillRank)
...
(All sub-documents use "Skillet" as the canonical product name.)
